// Configuration
const API_BASE_URL = "http://127.0.0.1:8000"; //"https://hitl-explanation.onrender.com";
...
function initializeEventListeners() {
  ...
  document
    .getElementById("explanationChangeCheckbox")
  ...
  document
    .getElementById("submitExplanationReview")
    .addEventListener("click", handleExplanationReviewSubmit);

  // Submit figure review
  document
    .getElementById("submitFigureReview")
    .addEventListener("click", handleFigureReviewSubmit);
          ...
          reader
            ...
            .then(({ done, value }) => {
              if (done) {
                ...
                if (
                  lastNodeExecuted === "fig_reviewer" &&
                  !screens.figureReview.classList.contains("active")
                ) {
                  console.log("Showing final review");
                  showFinalReview();
                }
                resolve();
                return;
                  ...
                  if (line.startsWith("event:")) {
                    ...
                    if (currentEvent.event && currentEvent.data) {
                      try {
                        ...
                        handleSSEEvent(currentEvent.event, parsedData).catch(
                          (err) => {
...
async function handleSSEEvent(eventName, data) {
  ...
  try {
    if (eventName === "__interrupt__") {
      ...
      // Just track the node, don't show final review here
...
async function loadAndShowExplanation(includeFigures) {
  console.log(
    "loadAndShowExplanation called with includeFigures:",
    includeFigures,
  );
  try {
    ...
    if (step.figure) {
      ...
      deleteButton.addEventListener("click", () => {
        if (figuresToDelete.has(figureId)) {
          ...
          document.getElementById(`figure-container-${index}`).style.opacity =
            "1";
          delete figureChangeDescriptions[figureId];
        } else {
          // Mark for deletion
...
async function handleUploadToCloud() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/crud/add_to_cloud/?concept_id=${currentConceptId}`,
      {
        method: "POST",
      },
    );
